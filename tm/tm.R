...
URL <- paste("http://api.digitalnz.org/v3/records.xml?api_key=LUq9-soDzWWhShuy3XhU", "","&and[category][]=Images&and[collection][]=New+Zealand+Cartoon+Archive&fields=id,shelf_location,source_url,title,description,collection", "",sep = "")
...
is_compound <- function(x) {
  ...
  if(check == TRUE) {result <- 0
                    return(result)}
  # if(check == 'Attributes: < Component “row.names”: 1 string mismatch >') {result <- 0
  #                                                                        return(result)}
  result <- mean(as.numeric(sub(".*?difference: (.*?)", "\\1", check)[2:length(check)]))
  if(result <- NA) {result <- 0} 
  return(result)
}
...
# length(all.equal(phi.t.df["john",], phi.t.df["key",]))
