...
parsing2 <- function(x){
  ...
  message(x, " is ", paste(lemma, sep="", collapse=" or "))
  return(lemma)}
...
translation_corpus <- research_corpus
...
## stemming
t1 <- Sys.time()
meaning_dictionary <- sapply(corpus_words, translating)

NumberOfForms <- max(unique(sapply(meaning_dictionary, length)))
number_lemmata <- sapply(meaning_dictionary, length)

t2 <- Sys.time()
translating_time <- t2 - t1

#correcting

lemmatiser2 <- function(x){
  lemmatised <- meaning_dictionary[[x]]
  return(lemmatised)}

t1 <- Sys.time()

temp <- strsplit(translation_corpus, " ")
temp_correct <- list()
for (i in 1:length(temp)) {
  temp_correct[[i]] <- sapply(temp[[i]], lemmatiser2) 
}
NumberOccurrences <- table(unlist(temp_correct))
...
translation_corpus <- unlist(corrected_corpus)

doc.list <- strsplit(translation_corpus, "[[:space:]]+")
t2 <- Sys.time()
correcting_time <- t2 - t1
...
translation_corpusAbstracts <- list(phi = phi,
                                 theta = theta,
                                 doc.length = doc.length,
                                 vocab = vocab,
                                 term.frequency = term.frequency)
...
# create the JSON object to feed the visualization:
json <- createJSON(phi = translation_corpusAbstracts$phi, 
                   theta = translation_corpusAbstracts$theta, 
                   doc.length = translation_corpusAbstracts$doc.length, 
                   vocab = translation_corpusAbstracts$vocab, 
                   term.frequency = translation_corpusAbstracts$term.frequency,
                   R=terms_shown)
