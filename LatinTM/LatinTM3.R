...
candidates_tis <- which(unlist(lapply(corpus[,2], FUN = function(x) {length(grep("tis\\W", unlist(strsplit(as.character(x), '[[:space:]]+'))[1:3]))})) == 1)
candidates_to <- which(unlist(lapply(corpus[,2], FUN = function(x) {length(grep("to\\W", unlist(strsplit(as.character(x), '[[:space:]]+'))[1:3]))})) == 1)
candidates_ta <- which(unlist(lapply(corpus[,2], FUN = function(x) {length(grep("ta\\W", unlist(strsplit(as.character(x), '[[:space:]]+'))[1:3]))})) == 1)
candidates <- unique(c(candidates_ta,candidates_tis,candidates_to))
candidates <- corpus[candidates,]
first_corpus <- read.table("LatinumB.csv", sep=",", header=TRUE)
identifiers <- c(as.character(first_corpus[,1]), candidates[,1])
texts <- c(as.character(first_corpus[,2]), candidates[,2])
AblAbsCaesar <- data.frame(identifiers, texts)
write.table(AblAbsCaesar, file = 'AblAbsCaesar.csv', append = FALSE, quote = FALSE, sep = ",", eol = "\n", na = "NA", dec = ".", row.names = FALSE, col.names = TRUE)
...
length_corpus <- length(identifiers)
...
serVis(json, out.dir = 'LatinumB_vis', open.browser = FALSE)

## get the tables

dir.create("LatinumB_tab")
...
write.table(theta.frame, file = 'LatinumB_tab/theta.csv', append = FALSE, quote = FALSE, sep = ",", eol = "\n", na = "NA", dec = ".", row.names = FALSE, col.names = TRUE)
...
position <- AblAbsCaesar[,1] %in% similar_sentences
similar_sentences.df <- data.frame(AblAbsCaesar[which(position == TRUE), 1], AblAbsCaesar[which(position == TRUE), 2], corpus_parsed[which(position == TRUE), 2])
colnames(similar_sentences.df) <- c(test_sentence, AblAbsCaesar[which(corpus == test_sentence), 2], corpus_parsed[which(corpus_parsed == test_sentence), 2])

base <- as.character(AblAbsCaesar[,1])[1:175]
base_parsed <- vector()
for (i in base){
...
for (i in 1:length(similar_sentences.df[,3])){
  ...
  hit_list[[i]] <- text_vector
  text_vector[position_parsed] <- paste("**", text_vector[position_parsed], "**", sep="")
  text_vector[position_notparsed] <- paste("[", text_vector[position_notparsed], "]", "(http://www.perseus.tufts.edu/hopper/morph?l=", text_vector[position_notparsed], "&la=lat#lexicon)", sep="")
...
filepath <- paste('LatinumB_tab/', test_sentence, '.csv', sep="")
write.table(similar_sentences.frame, file = filepath, append = FALSE, quote = FALSE, sep = ",", eol = "\n", na = "NA", dec = ".", row.names = FALSE, col.names = TRUE)

### Find in which known sentences known vocabulary occurs

base_parsed %in% "attingo" 
text_vector2 <- unlist(strsplit(paste(similar_sentences.df[i,2], sep = "", collapse = " "), "[[:space:]]+"))
