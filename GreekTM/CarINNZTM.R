...
first_corpus <- read.table("EQ-Cartoons-Doubles.csv", sep=",", header=TRUE)
...
keep <- c("gerry", "house", "second", 
          ...
          "council", "mayor", "large", 
          "cup", "government", "new", 
          "aftershocks", "first", "over", 
          "zone", "parker", "september", 
          "february", "city", "zealand", 
          "bob", "rugby")
stop_words2 <- stop_words2 [! stop_words2 %in% keep]
stop_words <- c(stopwords_english, stop_words2)
...
#Visulise and start browser
serVis(json, out.dir = 'CarINNZCompsRemoved_vis', open.browser = FALSE)

## get the tables

dir.create("CarINNZCompsRemoved_tab")
...
colnames(phi.t.df) <- phicolnames
write.table(phi.t.df, file = 'CarINNZCompsRemoved_tab/phi.csv', append = FALSE, quote = FALSE, sep = ",", eol = "\n", na = "NA", dec = ".", row.names = FALSE, col.names = TRUE)
...
thetacolnames[1] <- "identifier"
colnames(theta.frame) <- thetacolnames
write.table(theta.frame, file = 'CarINNZCompsRemoved_tab/theta.csv', append = FALSE, quote = FALSE, sep = ",", eol = "\n", na = "NA", dec = ".", row.names = FALSE, col.names = TRUE)

merge1 <- first_corpus[c(3,5,7,8,9,12)]
merge2 <- second_corpus[c(2,5,7,8,10,11)]
merge1 <- merge1[c(1,3,5,4,2,6)]
colnames(merge1) <- colnames(merge2)

base_corpus <- rbind(merge1, merge2)
total <- merge(base_corpus, theta.frame, by="identifier")


write.table(total, file = 'CarINNZCompsRemoved_tab/total.csv', append = FALSE, quote = FALSE, sep = ",", eol = "\n", na = "NA", dec = ".", row.names = FALSE, col.names = TRUE)
...
total_bydate <- total[order(total$Month),]
...
fusionsave <- total[-c(7,8,9)]
write.table(fusionsave, file = 'CarINNZCompsRemoved_tab/fusionsave.csv', append = FALSE, quote = FALSE, sep = ",", eol = "\n", na = "NA", dec = ".", row.names = FALSE, col.names = TRUE)

CarINNZData <- total_bydate

saveRDS(CarINNZData, file="CarINNZData.Rda")